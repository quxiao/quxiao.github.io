...
首先是一些基本的业务逻辑，比如自然排序下，有货的商品排在前面，无货的在后面；SPU商品排在前面，SKU商品排在后面。（SKU、SPU的概念后面会讲到）

另外还有一些运营方面的考虑。比如，发现搜索结果中有一个很不相关的商品出现，这时就急需在query粒度上对这个商品进行打压、甚至是不允许展现。或者，由于某种合作关系，在某些query或者分类下，必须将某个商家的商品排在前面。因此，检索系统后台就需要维护这么一份商品的“黑白名单”。


标签聚合
------
所谓标签，就是用一些“键-值”的概念来描述一个商品。比如说MacBookPro，可以有如下标签：
...
当用户检索商品时，检索系统除了直接展示商品以外，还会将商品上面的标签进行聚合，一般都是通过“标签名 + 标签值的列表”的形式展现给用户（如下图），给方便用户通过标签进行进一步的筛选。
...
一般来说，检索系统为了保证query的准确率，会在检索条件中添加预测分类，使得检索结果不至于各种分类的商品混杂在一起，影响用户体验。所以当用户搜索“小米”时，检索结果会限定在“手机通讯”这个分类下，但是如果用户真的是想搜索“粮油米面”下的小米，也没关系，只需在分类树中进行限定即可。
...
以下是京东的query提示截屏，有拼音翻译为query、有分类预测、有每个query对应的检索商品数。
...
相对而言，微购做的query提示就原始许多，输入“shouji”，居然连基本的“手机”都没有，囧……
...
根据我在网上查找到的资料，SKU是**Stock Keeping Unit**，指的是*库存的最小单位*；而SPU是**Standard Product Unit**，是指*商品信息聚合的最小单位，是一组可复用、易检索的标准化信息的集合，该集合描述了一个产品的特性*。

简单的理解就是，“iPhone4S”是一个SPU，“iPhone4S 白色 16G 电信版”就是一个SKU；“MacBookPro”是一个SPU，“MacBookPro 13寸 8G内存 128G硬盘”就是一个SKU。

因为，当用户进行商品检索时，需要将SKU粒度的商品聚合成SPU粒度，使得检索结果比较多样，从而不至于满屏都是各种颜色、型号的同一款商品。等到用户进行商品详情页之后，再来选择具体的型号。
